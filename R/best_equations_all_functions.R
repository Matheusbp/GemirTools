# ============================================================================
# Compêndio de Equações de Biomassa para Florestas do Brasil
# Funções geradas a partir de _best_equations.csv
# ============================================================================
...
# Google Sheets: 0.4049*(DBH^2.3166)*(hf^0.0221)
acacia_mangium_willd_amazonia <- function(dap, hf, porc_C = 0.47) {
  return(porc_C * 0.4049*(dap^2.3166)*(hf^0.0221))
}
...
# Google Sheets: 0.31046*(DBH^1.54806)*(HT^0.78635)
...
# Estado: AM | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 1,00 | Syx: - | Peso: Seco
...
# Google Sheets: EXP(-2.4387+2.0751*LN(DBH)+0.5360*LN(HT))
...
# Estado: AM | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,99 | Syx: 13,8% | Peso: Seco
...
# Google Sheets: 11.247216-
# 3.801515*DBH+0.301382*DBH^2+0.020887*DBH^2*HT
...
# Estado: AM | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,98 | Syx: - | Peso: Seco
# DBH range (cm): 5,1 a 382 | Autor: Nelson et al. (1999)
# referência: NELSON, B. W. et al. Allometric regressions for improved estimate of secondary forest biomass in the central Amazon. Forest Ecology e Management, v. 117, p. 149–167, 1999.
# Google Sheets: EXP(-2.5118+2.4257*LN(DBH))
...
# Estado: AM | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,99 | Syx: - | Peso: Seco
# DBH range (cm): 2,3 a 253 | Autor: Nelson et al. (1999)
# referência: NELSON, B. W. et al. Allometric regressions for improved estimate of secondary forest biomass in the central Amazon. Forest Ecology e Management, v. 117, p. 149–167, 1999.
# Google Sheets: EXP(-2.7009+2.5996*LN(DBH))
...
# referência: NELSON, B. W. et al. Allometric regressions for improved estimate of secondary forest biomass in the central Amazon. Forest Ecology e Management, v. 117, p. 149–167, 1999.
# Google Sheets: EXP(-2.4990+2.1283*LN(DBH)+0.5508*LN(HT))
...
# Google Sheets: 2.928*(DBH^3)-37.554*(DBH^2)+161.23*DBH-226.54
...
# referência: ROBORTELLA, H. S. Equações de biomassa e estoques de carbono de seis espécies em plantios mistos no noroeste do Mato Grosso. Instituto Nacional de Pesquisas da Amazônia (INPA), 2010.
# Google Sheets: EXP(-1.888+2.119*LN(DBH)-0.474*LN(hf/HT))
...
# referência: NELSON, B. W. et al. Allometric regressions for improved estimate of secondary forest biomass in the central Amazon. Forest Ecology e Management, v. 117, p. 149–167, 1999.
# Google Sheets: EXP(-2.6385+2.1976*LN(DBH)+0.4565*LN(HT))
...
# DBH range (cm): 10,0 a 230 | Autor: Robortella (2010)
# referência: ROBORTELLA, H. S. Equações de biomassa e estoques de carbono de seis espécies em plantios mistos no noroeste do Mato Grosso. Instituto Nacional de Pesquisas da Amazônia (INPA), 2010.
# Google Sheets: EXP(-2.450+2.607*LN(DBH))
...
# Várias | Amazônia
# Estado: AM | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,93 | Syx: 5,6% | Peso: Seco
...
# referência: NELSON, B. W. et al. Allometric regressions for improved estimate of secondary forest biomass in the central Amazon. Forest Ecology e Management, v. 117, p. 149–167, 1999.
# Google Sheets: EXP(-2.3706+2.5392*LN(DBH))
...
# referência: NELSON, B. W. et al. Allometric regressions for improved estimate of secondary forest biomass in the central Amazon. Forest Ecology e Management, v. 117, p. 149–167, 1999.
# Google Sheets: EXP(-1.7829+2.3651*LN(DBH))
...
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 93,45 | Syx: 15,1% | Peso: Fresco
...
# Google Sheets: 51.63157/(1+16.05867*EXP(-0.362138*DBH))
anadenanthera_colubrina_vell_brenan_caatinga <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 51.63157/(1+16.05867*exp(-0.362138*dap)))
}

# Anadenanthera colubrina var. cebil (Griseb.) Altschul | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,94 | Syx: - | Peso: Seco
...
# Google Sheets: 0.2482*(DBH^2.1628)
anadenanthera_colubrina_var_cebil_griseb_altschul_caatinga <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 0.2482*(dap^2.1628))
}

# Aspidosperma pyrifolium Mart. & Zucc. | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,75 | Syx: 26,6% | Peso: Seco
...
# Google Sheets: 0.7858*((DBH^2)*HT)^0.4550
aspidosperma_pyrifolium_mart_zucc_caatinga <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 0.7858*((dap^2)*altura)^0.4550)
}

# Astronium urundeuva 
# (M. Allemão) Engl. | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,98 | Syx: - | Peso: Seco
# DBH range (cm): 2,0 a 500 | Autor: Sampaio e Silva (2005)
# referência: SAMPAIO, E. V. S. B.; SILVA, G. C. Biomass equations for Brazilian semiarid caatinga plants. Acta Botanica Brasilica, v. 19, n. 4, p. 935–943, 2005.
# Google Sheets: 0.1397*(DBH^2.4659)
astronium_urundeuva_m_allemao_engl_caatinga <- function(dap, altura, porc_C = 0.47) {
...
# (Bong.) Steud. | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,97 | Syx: 12,1% | Peso: Seco
# DBH range (cm): 1,9 a 95 | Autor: Dalla Lana et al. (2018)
# referência: DALLA LANA, M. et al. Biomass equations for caatinga species. Nativa, v. 6, n. 5, p. 517, 2018.
# Google Sheets: 0.0699*(DBH^2.2115)*(HT^0.8155)
...
# (Tul.) E. Gagnon & G.P. Lewis | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,85 | Syx: 23,4% | Peso: Seco
# DBH range (cm): 1,9 a 95 | Autor: Dalla Lana et al. (2018)
# referência: DALLA LANA, M. et al. Biomass equations for caatinga species. Nativa, v. 6, n. 5, p. 517, 2018.
# Google Sheets: 6.6205+0.0341*((DBH^2)*HT)
...
# (Tul.) E. Gagnon & G.P. Lewis | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,82 | Syx: - | Peso: Seco
# DBH range (cm): 2,0 a 500 | Autor: Sampaio e Silva (2005)
# referência: SAMPAIO, E. V. S. B.; SILVA, G. C. Biomass equations for Brazilian semiarid caatinga plants. Acta Botanica Brasilica, v. 19, n. 4, p. 935–943, 2005.
# Google Sheets: 0.2365*(DBH^2.1928)
cenostigma_pyramidale_tul_e_gagnon_g_p_lewis_caatinga <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 0.2365*(dap^2.1928))
}

# Cereus jamacaru DC. | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,70 | Syx: - | Peso: Seco
# DBH range (cm): 2,0 a 500 | Autor: Sampaio e Silva (2005)
# referência: SAMPAIO, E. V. S. B.; SILVA, G. C. Biomass equations for Brazilian semiarid caatinga plants. Acta Botanica Brasilica, v. 19, n. 4, p. 935–943, 2005.
# Google Sheets: 0.0010*(DBH^3.2327)
cereus_jamacaru_dc_caatinga <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 0.0010*(dap^3.2327))
}

# Cnidoscolus quercifolius Pohl | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,72 | Syx: 17,5% | Peso: Seco
# DBH range (cm): 1,9 a 95 | Autor: Dalla Lana et al. (2018)
# referência: DALLA LANA, M. et al. Biomass equations for caatinga species. Nativa, v. 6, n. 5, p. 517, 2018.
# Google Sheets: EXP(-0.6664+0.5237*LN((DBH^2)*HT))
...
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,76 | Syx: 19,0% | Peso: Seco
# DBH range (cm): 1,9 a 95 | Autor: Dalla Lana et al. (2018)
# referência: DALLA LANA, M. et al. Biomass equations for caatinga species. Nativa, v. 6, n. 5, p. 517, 2018.
# Google Sheets: 0.1868*(DBH^1.2764)*(HT^0.9401)
...
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,84 | Syx: - | Peso: Seco
# DBH range (cm): 2,0 a 500 | Autor: Sampaio e Silva (2005)
# referência: SAMPAIO, E. V. S. B.; SILVA, G. C. Biomass equations for Brazilian semiarid caatinga plants. Acta Botanica Brasilica, v. 19, n. 4, p. 935–943, 2005.
# Google Sheets: 0.3569*(DBH^1.8565)
croton_jacobinensis_baill_caatinga <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 0.3569*(dap^1.8565))
}

# Dalbergia cearensis Ducke | Caatinga
# Estado: CE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,67 | Syx: - | Peso: Seco
# DBH range (cm): 5,0 a 201 | Autor: Nogueira et al. (2021)
# Google Sheets: EXP(-3.710+1.878*LN(DBH*HT))
...
# (Pohl) Baill. | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,83 | Syx: - | Peso: Seco
# DBH range (cm): 2,0 a 500 | Autor: Sampaio e Silva (2005)
# referência: SAMPAIO, E. V. S. B.; SILVA, G. C. Biomass equations for Brazilian semiarid caatinga plants. Acta Botanica Brasilica, v. 19, n. 4, p. 935–943, 2005.
# Google Sheets: 0.1970*(DBH^1.8145)
jatropha_mollissima_pohl_baill_caatinga <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 0.1970*(dap^1.8145))
}

# Mimosa caesalpiniifolia Benth. | Caatinga
# Estado: RN | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,71 | Syx: 13,7% | Peso: Fresco
# DBH range (cm): 5,2 a 82 | Autor: Lucena et al. (2020)
# Google Sheets: EXP(-0.119429+LN(DBH)*1.527954)
mimosa_caesalpiniifolia_benth_caatinga <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * exp(-0.119429+log(dap)*1.527954))
}

# Mimosa hostilis (Mart.) Benth. | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,96 | Syx: - | Peso: Seco
# DBH range (cm): 2,0 a 500 | Autor: Sampaio e Silva (2005)
# referência: SAMPAIO, E. V. S. B.; SILVA, G. C. Biomass equations for Brazilian semiarid caatinga plants. Acta Botanica Brasilica, v. 19, n. 4, p. 935–943, 2005.
# Google Sheets: 0.3127*(DBH^2.1183)
mimosa_hostilis_mart_benth_caatinga <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 0.3127*(dap^2.1183))
}

# Mimosa ophthalmocentra Mart. ex Benth. | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,88 | Syx: 9,0% | Peso: Seco
# DBH range (cm): 1,9 a 95 | Autor: Dalla Lana et al. (2018)
# referência: DALLA LANA, M. et al. Biomass equations for caatinga species. Nativa, v. 6, n. 5, p. 517, 2018.
# Google Sheets: EXP(1.1118+1.7371*LN(DBH)-0.9536*LN(HT))
...
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,92 | Syx: 10,6% | Peso: Seco
# DBH range (cm): 1,9 a 95 | Autor: Dalla Lana et al. (2018)
# referência: DALLA LANA, M. et al. Biomass equations for caatinga species. Nativa, v. 6, n. 5, p. 517, 2018.
# Google Sheets: EXP(-1.6100+0.7850*LN((DBH^2)*HT))
...
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,95 | Syx: - | Peso: Seco
# DBH range (cm): 2,0 a 500 | Autor: Sampaio e Silva (2005)
# referência: SAMPAIO, E. V. S. B.; SILVA, G. C. Biomass equations for Brazilian semiarid caatinga plants. Acta Botanica Brasilica, v. 19, n. 4, p. 935–943, 2005.
# Google Sheets: 0.3460*(DBH^2.0231)
monteverdia_rigida_mart_biral_caatinga <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 0.3460*(dap^2.0231))
}

# Schinopsis brasiliensis Engl. | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,97 | Syx: - | Peso: Seco
# DBH range (cm): 2,0 a 500 | Autor: Sampaio e Silva (2005)
# referência: SAMPAIO, E. V. S. B.; SILVA, G. C. Biomass equations for Brazilian semiarid caatinga plants. Acta Botanica Brasilica, v. 19, n. 4, p. 935–943, 2005.
# Google Sheets: 0.2274*(DBH^2.2710)
schinopsis_brasiliensis_engl_caatinga <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 0.2274*(dap^2.2710))
}

# Várias | Caatinga
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,85 | Syx: 23,5% | Peso: Seco
# DBH range (cm): 1,9 a 95 | Autor: Dalla Lana et al. (2018)
# referência: DALLA LANA, M. et al. Biomass equations for caatinga species. Nativa, v. 6, n. 5, p. 517, 2018.
# Google Sheets: EXP(-1.2884+1.6102*LN(DBH)+0.4343*LN(HT))
...
# Estado: MG | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 94,99 | Syx: 21,2% | Peso: Seco
...
# Google Sheets: EXP(-9.995149073+2.4884117855*LN(DBH)+0.3203838261*LN(HT))*1000
...
# Estado: SP | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,98 | Syx: 20,0% | Peso: Seco
...
# Google Sheets: EXP(-2.8564+2.4771*LN(DBH))
croton_floribundus_spreng_cerrado <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * exp(-2.8564+2.4771*log(dap)))
}

# Eremanthus erythropappus(DC.) MacLeish | Cerrado
# Estado: MG | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 98,48 | Syx: 20,7% | Peso: Seco
# DBH range (cm): 5,0 a 350 | Autor: Scolforo et al. (2008)¹
# referência: SCOLFORO, J. R. et al. Inventário Florestal de Minas Gerais-Equações de Volume, Peso de Matéria Seca e Carbono para Diferentes Fisionomias da Flora Nativa. Editora UFLA, 2008.
# Google Sheets: EXP(-3.6776993+1.0141433*LN((DBH^2)*HT))
...
# Eucalyptus camaldulensisDehnh. | Cerrado
# Estado: MG | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,97 | Syx: 11,5% | Peso: Seco
...
# Google Sheets: 0.002222-0.000062*(DBH^2)+0.00002*(DBH^2*HT)+0.000001405*(DBH* HT^2)
...
# Estado: MG | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: - | Syx: 6,7% | Peso: Seco
...
# referência: RIBEIRO, S. C. et al. Biomassa acima e abaixo do solo e estimativas de carbono para um plantio clonal de eucalipto no Sudeste do Brasil. Revista Árvore, v. 39, n. 2, p. 353–363, 2015.
# Google Sheets: 0.0249*(((DBH^2)*HT)^0.9679)
eucalyptus_grandis_w_hill_cerrado <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 0.0249*(((dap^2)*altura)^0.9679))
}

# Kielmeyera 10oriáceaMart. & Zucc. | Cerrado
# Estado: MG | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 96,09 | Syx: 21,2% | Peso: Seco
# DBH range (cm): 5,0 a 250 | Autor: Scolforo et al. (2008)¹
# Google Sheets: EXP(- 11.2626907253+2.428844827*LN(DBH)+0.8160943744*LN(HT))*1000
...
# Mart. & Zucc. | Cerrado
# Estado: MG | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 97,64 | Syx: 12,7% | Peso: Seco
# DBH range (cm): 5,0 a 250 | Autor: Scolforo et al. (2008)¹
# Google Sheets: EXP(-10.454935288+2.1243668209*LN(DBH)+0.4846364811*LN(HT))*1000
...
# Estado: MG | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 96,48 | Syx: 24,0% | Peso: Seco
# DBH range (cm): 5,0 a 250 | Autor: Scolforo et al. (2008)¹
# Google Sheets: EXP(-11.2875746052+2.3822964052*LN(DBH)+0.9086217316*LN(HT))*1000
...
# Stryphnodendron adstringens(Mart.) Coville | Cerrado
# Estado: MG | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 98,68 | Syx: 10,8% | Peso: Seco
# DBH range (cm): 5,0 a 250 | Autor: Scolforo et al. (2008)¹
# Google Sheets: EXP(-11.3841308978+2.9100586552*LN(DBH)+0.1989771767*LN(HT))*1000
stryphnodendron_adstringens_mart_coville_cerrado <- function(dap, altura, porc_C = 0.47) {
...
# Google Sheets: EXP(-10.1966777197+2.4240650757*LN(DBH)+0.5413156008*LN(HT))*1000
...
# Estado: MG | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 94,50 | Syx: 28,8% | Peso: Seco
# DBH range (cm): 10,0 a 400 | Autor: Scolforo et al. (2008)¹
# Google Sheets: EXP(-9.341381587+2.27623091*LN(DBH)+0.3954337202*LN(HT))*1000
...
# Google Sheets: EXP(-2.12394+2.46033*LN(DBH))
araucaria_angustifolia_bertol_kuntze_mata_atlantica <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * exp(-2.12394+2.46033*log(dap)))
}

# Ateleia glazioveana Baill. | Mata Atlântica
# Estado: RS | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,85 | Syx: 31,4% | Peso: Seco
...
# Google Sheets: -5.6260+1.2986*LN(DBH^2*HT)
ateleia_glazioveana_baill_mata_atlantica <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * (-5.6260+1.2986*log(dap^2*altura)))
}

# Avicennia schaueriana 
# Stapf & Leechm. ex Moldenke | Mata Atlântica
# Estado: RJ | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,99 | Syx: 17,4% | Peso: Seco
...
# Google Sheets: 68.8881*((DBH^2)*HT)^0.9397/1000
...
# Estado: SP | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,78 | Syx: 14,6% | Peso: Seco
...
# Google Sheets: -13.4200+3.9572*DBH
bambusa_oldhamii_munro_mata_atlantica <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * (-13.4200+3.9572*dap))
}

# Bambusa vulgaris
# Schrad. ex J.C.Wendl. | Mata Atlântica
# Estado: SP | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,76 | Syx: 16,5% | Peso: Seco
# DBH range (cm): 5,7 a 100 | Autor: Sanquetta et al. (2015a)
# referência: SANQUETTA, C. R. et al. Biomassa Individual De Bambusa oldhamii Munro E Bambusa vulgaris Schrad. Ex J.C. Wendl. Cerne, v. 21, n. 1, p. 151–159, 2015a.
# Google Sheets: 4.9887+0.0144*((DBH^2)*HT)
...
# Estado: SC | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,98 | Syx: 7,7 kg | Peso: Seco
...
# Google Sheets: 295.6/(1+EXP(6.1366-0.1161*DBH-0.1522*HT))
cecropia_glaziovii_snethl_mata_atlantica <- function(dap, porc_C = 0.47) {
  return(porc_C * 295.6/(1+exp(6.1366-0.1161*dap-0.1522*altura)))
}

# Cryptomeria japonica
# (Thunb. Ex L.f.) D.Don | Mata Atlântica
# Estado: PR | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,96 | Syx: 11,1% | Peso: Seco
...
# Google Sheets: 5.81707+0.01679*DBH^2*HT
...
# Estado: SC | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,92 | Syx: 3,6 kg | Peso: Seco
# DBH range (cm): 6,3 a 195 | Autor: Uller et al. (2021)
# referência: ULLER, H. F. et al. Biomass models for three species with different growth forms e geographic distribution in the Brazilian Atlantic forest. Canadian Journal of Forest Research, v. 51, n. 10, p. 1419–1431, 2021.
# Google Sheets: 0.1523*DBH^1.1254*HT^1.0338
...
# Estado: SC | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,84 | Syx: 7,1 kg | Peso: Seco
# DBH range (cm): 3,9 a 158 | Autor: Uller et al. (2021)
# referência: ULLER, H. F. et al. Biomass models for three species with different growth forms e geographic distribution in the Brazilian Atlantic forest. Canadian Journal of Forest Research, v. 51, n. 10, p. 1419–1431, 2021.
# Google Sheets: 0.0175*DBH^1.5288*HT^1.66
euterpe_edulis_mart_mata_atlantica <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 0.0175*dap^1.5288*altura^1.66)
}

# Guadua chacoensis 
# (Rojas) Londoño | Mata Atlântica
# Estado: PR | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,86 | Syx: 20,5% | Peso: Seco
...
# Google Sheets: EXP(0.62186+0.35049*LN(HT^2))
...
# (Rojas) Londoño | Mata Atlântica
# Estado: PR | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,93 | Syx: 15,0% | Peso: Seco
# DBH range (cm): 5,6 a 140 | Autor: Mognon et al. (2014)
# referência: MOGNON, F. et al. Estimativas de biomassa para plantas de bambu do gênero Guadua. Revista Ceres, v. 61, n. 6, p. 900–906, 2014.
# Google Sheets: -27.53382+2.80989*DBH+1.91081*HT
guadua_chacoensis_rojas_londo_o_mata_atlantica <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * (-27.53382+2.80989*dap+1.91081*altura))
}

# Laguncularia racemosa 
# (L.) C.F.Gaertn. | Mata Atlântica
# Estado: PE | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,97 | Syx: - | Peso: Seco
...
# Google Sheets: 0.1214*((DBH^2)*HT)^0.8615
...
# Google Sheets: EXP(3.4756+0.22*HT-0.028*hf+1.4425*LN(cap))/1000
leucaena_leucocephala_lam_de_wit_mata_atlantica <- function(dap, altura, porc_C = 0.47) {
...
# Google Sheets: -0.49361+0.034865*(DBH^2)*HT
...
# Google Sheets: EXP(-2.08721+0.0364642*DBH+0.71874*LN(DBH^2*HT))
...
# Phyllostachys aurea 
# Rivière & C. Rivière. | Mata Atlântica
# Estado: PR | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,65 | Syx: 30,2% | Peso: Seco
# DBH range (cm): 1,4 a 46 | Autor: Monastier et al. (2015)
# Google Sheets: (572.612+0.155*((DBH*10)^2)*HT)/1000
...
# Estado: PR | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,98 | Syx: 5,7% | Peso: Seco
# DBH range (cm): 6,0 a 120 | Autor: Girard (2005)
# Google Sheets: 0.08562+5.147161*hf+0.364603*(DBH^2)-0.08531*((DBH^2)*hf)
pimenta_pseudocaryophyllus_gomes_landrum_mata_atlantica <- function(dap, altura, porc_C = 0.47) {
...
# Estado: BA | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 84,42 | Syx: 25,6% | Peso: Seco
# DBH range (cm): 5,8 a 238 | Autor: Lima et al. (2016)
# Google Sheets: 5.19+0.03*((DBH^2)*HT)
...
# Google Sheets: -12.9265+1.4294*DBH+0.0185*(DBH^2)*HT
...
# Estado: RS | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,98 | Syx: 1,5% | Peso: Seco
# DBH range (cm): 18,1 a 508 | Autor: Witschoreck (2008)
# Google Sheets: EXP(7.854652-63.900403*(DBH^-1))
...
# Google Sheets: EXP(5.360943+2.380682*LN(DBH))/1000
rhizophora_mangle_l_mata_atlantica <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * exp(5.360943+2.380682*log(dap))/1000)
}

# Tabebuia cassinoides 
# (Lam.) DC. | Mata Atlântica
# Estado: PR | Compartimento: Total (abaixo+acima) | R2: 0,96 | Syx: 7,9% | Peso: Seco
# DBH range (cm): 8,0 a 159 | Autor: Melo et al. (2014)
# Google Sheets: -25.095+4.858*DBH
tabebuia_cassinoides_lam_dc_mata_atlantica <- function(dap, altura, porc_C = 0.47) {
...
# DBH range (cm): 8,0 a 159 | Autor: Melo et al. (2014)
# Google Sheets: 11.399+0.011*(DBH^3)
tabebuia_cassinoides_lam_dc_mata_atlantica <- function(dap, altura, porc_C = 0.47) {
...
# Google Sheets: 10^(-0.882390231+2.409594057*LOG10(DBH))
varias_mata_atlantica <- function(dap, altura, porc_C = 0.47) {
...
# Attalea phalerata 
# Mart. ex Spreng. | Outro
# Estado: Outro | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,86 | Syx: - | Peso: Seco
...
# Google Sheets: EXP(3.2579+1.1249*LN(hf+1))
attalea_phalerata_mart_ex_spreng_outro <- function(dap, altura, porc_C = 0.47) {
...
# referência: GOODMAN, R. C. et al. Amazon palm biomass e allometry. Forest Ecology e Management, v. 310, p. 994–1004, 2013.
# Google Sheets: -108.81+13.589*hf
euterpe_precatoria_mart_outro <- function(dap, altura, porc_C = 0.47) {
...
# referência: GOODMAN, R. C. et al. Amazon palm biomass e allometry. Forest Ecology e Management, v. 310, p. 994–1004, 2013.
# Google Sheets: EXP(-3.483+0.94371*LN((DBH^2)*hf))
iriartea_deltoidea_ruiz_pav_outro <- function(dap, altura, porc_C = 0.47) {
...
# referência: GOODMAN, R. C. et al. Amazon palm biomass e allometry. Forest Ecology e Management, v. 310, p. 994–1004, 2013.
# Google Sheets: EXP(-0.3688+2.0106*LN(hf))
mauritia_flexuosa_l_f_outro <- function(dap, altura, porc_C = 0.47) {
...
# referência: GOODMAN, R. C. et al. Amazon palm biomass e allometry. Forest Ecology e Management, v. 310, p. 994–1004, 2013.
# Google Sheets: EXP(1.0945+0.11086*hf)
mauritiella_aculeata_kunth_burret_outro <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * exp(1.0945+0.11086*hf))
}

# Oenocarpus bataua Mart. | Outro
# Estado: Outro | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,78 | Syx: - | Peso: Seco
# DBH range (cm): 21,0 a 410 | Autor: Goodman et al. (2013)
# referência: GOODMAN, R. C. et al. Amazon palm biomass e allometry. Forest Ecology e Management, v. 310, p. 994–1004, 2013.
# Google Sheets: EXP(4.5496+0.1387*hf)
oenocarpus_bataua_mart_outro <- function(dap, altura, porc_C = 0.47) {
...
# referência: GOODMAN, R. C. et al. Amazon palm biomass e allometry. Forest Ecology e Management, v. 310, p. 994–1004, 2013.
# Google Sheets: EXP(-3.7965+1.0029*LN((DBH^2)*hf))
socratea_exorrhiza_mart_h_wendl_outro <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * exp(-3.7965+1.0029*log((dap^2)*hf)))
}

# Várias | Outro
# Estado: Outro | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,87 | Syx: 14,0% | Peso: Seco
...
# Google Sheets: EXP(-6.93+1.86*LN(DBH)+0.0045*LN(DBH)*HT)*1000
...
# Google Sheets: 10^(-1.2874+2.80262*LOG10(DBH))
acacia_mearnsii_de_wild_pampa <- function(dap, altura, porc_C = 0.47) {
...
# Google Sheets: -76.809+14.899*DBH
anadenanthera_colubrina_vell_brenan_pantanal <- function(dap, altura, porc_C = 0.47) {
...
# referência: PADILHA, D. R. C. Fitossociologia e estimativas da biomassa aérea e de carbono em chaco florestado no Brasil. Universidade Federal de Mato Grosso do Sul, 2011.
# Google Sheets: 6.717*EXP(DBH*0.139)
coccoloba_guaranitica_hassler_pantanal <- function(dap, altura, porc_C = 0.47) {
...
# referência: PADILHA, D. R. C. Fitossociologia e estimativas da biomassa aérea e de carbono em chaco florestado no Brasil. Universidade Federal de Mato Grosso do Sul, 2011.
# Google Sheets: 0.306*(DBH^2.145)
diplokeleba_floribunda_n_e_br_pantanal <- function(dap, altura, porc_C = 0.47) {
...
# Google Sheets: EXP(-2.119+2.38*LN(DBH))
diptychandra_aurantiaca_tul_pantanal <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * exp(-2.119+2.38*log(dap)))
}

# Licania minutiflora 
# (Sagot) Fritsch | Pantanal
# Estado: MS | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,91 | Syx: - | Peso: Seco
...
# Google Sheets: EXP(-2.265+2.386*LN(DBH))
licania_minutiflora_sagot_fritsch_pantanal <- function(dap, altura, porc_C = 0.47) {
...
# referência: SALIS, S. M. et al. Estimating the aboveground biomass e wood volume of savanna woodlands in Brazil's Pantanal wetlands based on allometric correlations. Forest Ecology e Management, v. 228, n. 1–3, p. 61–68, 2006.
# Google Sheets: EXP(-2.888+2.795*LN(DBH))
magonia_pubescens_a_st_hil_pantanal <- function(dap, altura, porc_C = 0.47) {
...
# referência: PADILHA, D. R. C. Fitossociologia e estimativas da biomassa aérea e de carbono em chaco florestado no Brasil. Universidade Federal de Mato Grosso do Sul, 2011.
# Google Sheets: 5.886*EXP(DBH*0.127)
phyllostylon_rhamnoides_poiss_taub_pantanal <- function(dap, altura, porc_C = 0.47) {
  return(porc_C * 5.886*exp(dap*0.127))
}

# Protium heptaphyllum 
# (Aubl.) Marchand | Pantanal
# Estado: MS | Compartimento: Acima 
# (tronco+galhos+folhas) | R2: 0,97 | Syx: - | Peso: Seco
# DBH range (cm): 8,0 a 360 | Autor: Salis et al. (2006)
# referência: SALIS, S. M. et al. Estimating the aboveground biomass e wood volume of savanna woodlands in Brazil's Pantanal wetlands based on allometric correlations. Forest Ecology e Management, v. 228, n. 1–3, p. 61–68, 2006.
# Google Sheets: EXP(-2.083+2.536*LN(DBH))
protium_heptaphyllum_aubl_marchand_pantanal <- function(dap, altura, porc_C = 0.47) {
...
# referência: SALIS, S. M. et al. Estimating the aboveground biomass e wood volume of savanna woodlands in Brazil's Pantanal wetlands based on allometric correlations. Forest Ecology e Management, v. 228, n. 1–3, p. 61–68, 2006.
# Google Sheets: EXP(-1.915+2.409*LN(DBH))
terminalia_argentea_mart_zucc_pantanal <- function(dap, altura, porc_C = 0.47) {
...
# referência: PADILHA, D. R. C. Fitossociologia e estimativas da biomassa aérea e de carbono em chaco florestado no Brasil. Universidade Federal de Mato Grosso do Sul, 2011.
# Google Sheets: 0.188*(DBH^2.266)
varias_pantanal <- function(dap, altura, porc_C = 0.47) {
